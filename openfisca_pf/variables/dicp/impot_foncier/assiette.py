...
class valeur_locative_loyers(Variable):
    ...
    def formula_1950_11_16(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        loyer_de_janvier = personne('loyer_de_janvier', period)
...
class taux_archipel(Variable):
    ...
    def formula_1950_11_16(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        ...
        taux_archipel_iles_du_vent = personne('taux_archipel_iles_du_vent', period)
        taux_autres_archipels = personne('taux_autres_archipels', period)
        return where(
    ...
    def formula_2011_09_27(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        archipel = personne('archipel', period)
        taux_archipel_australes = personne('taux_archipel_australes', period)
        taux_archipel_gambiers = personne('taux_archipel_gambiers', period)
        taux_archipel_iles_du_vent = personne('taux_archipel_iles_du_vent', period)
        taux_archipel_iles_sous_le_vent = personne('taux_archipel_iles_sous_le_vent', period)
        taux_archipel_marquises = personne('taux_archipel_marquises', period)
        taux_archipel_tuamotus = personne('taux_archipel_tuamotus', period)
        return select(
            [
...
class valeur_locative_direct(Variable):
    ...
    def formula_1950_11_16(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        ...
        taux_archipel = personne('taux_archipel', period)
...
class valeur_locative_sociale(Variable):
    ...
    def formula_2001_11_13(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        valeur_venale = personne('valeur_venale', period)
        taux_logement_social = personne('taux_logement_social', period)
...
class valeur_locative_meuble_de_tourisme(Variable):
    ...
    def formula_1950_11_16(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        valeur_venale = personne('valeur_venale', period)
        taux_meuble_de_tourisme = personne('taux_meuble_de_tourisme', period)
...
class valeur_locative_villa_de_luxe(Variable):
    ...
    def formula_1950_11_16(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        valeur_venale = personne('valeur_venale', period)
        taux_villa_de_luxe = personne('taux_villa_de_luxe', period)
...
class valeur_locative_brute(Variable):
    ...
    def formula_1950_11_16(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        ...
        location_simple = personne('location_simple', period)
        location_meuble_de_tourisme = personne('location_meuble_de_tourisme', period)
        location_villa_de_luxe = personne('location_villa_de_luxe', period)
        non_loue = personne('non_loue', period)

        # Differents calculs de la valeur locative
        valeur_locative_loyers = personne('valeur_locative_loyers', period)
        valeur_locative_meuble_de_tourisme = personne('valeur_locative_meuble_de_tourisme', period)
        valeur_locative_villa_de_luxe = personne('valeur_locative_villa_de_luxe', period)
        valeur_locative_direct = personne('valeur_locative_direct', period)
    ...
    def formula_2001_11_13(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        ...
        valeur_locative_loyers = personne('valeur_locative_loyers', period)
        valeur_locative_direct = personne('valeur_locative_direct', period)
        valeur_locative_sociale = personne('valeur_locative_sociale', period)
        valeur_locative_meuble_de_tourisme = personne('valeur_locative_meuble_de_tourisme', period)
        valeur_locative_villa_de_luxe = personne('valeur_locative_villa_de_luxe', period)
