...
class base_imposable_it_ventes(Variable):
    value_type = float
    entity = Personne
...
class base_imposable_it_ventes_sans_abattement_droits(Variable):
    value_type = float
    entity = Personne
    definition_period = YEAR
    ...
    def formula(personne: Personne, period: Period, parameters: Parameters) -> ArrayLike:
        ...
        for activite in [*parameters(period).dicp.abattements_it_cstns.activites_ventes]:
            ...
            abattement_de_droit_de_charge_applicable = abattement_droits_charges \
                * (chiffre_d_affaire > seuil_bascule_abattement_de_droit) \
