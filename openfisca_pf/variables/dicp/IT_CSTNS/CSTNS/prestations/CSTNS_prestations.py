...
class cstns_prestations_avant_abattement_droits(Variable):
    ...
    #     base_imposable_cstns_ventes = entreprise('base_imposable_cstns_ventes', period) / 4
    #     echelle = parameters(period).dicp.cst_ns.taux_prestations
    #     ca = entreprise('base_imposable_cstns_prestations', period)
    #     return round_(echelle.calc(base_imposable_cstns_ventes + ca))
    def formula(entreprise, period, parameters):
        value = 0
        nombre_tranches_cstns_prestations = entreprise.pays('nombre_tranches_cstns_prestations', period)[0]
        for i in range(1, nombre_tranches_cstns_prestations + 1):
            value += entreprise(f'montant_cstns_prestations_du_tranche_{i}', period)
        return value
...
class cstns_prestations_sans_abattement_droits(Variable):
    ...
    def formula(entreprise, period, parameters):
        base_imposable_cstns_ventes = entreprise('base_imposable_cstns_ventes', period) / 4
        bareme = creerBareme(entreprise, period, 'cstns', 'prestations')
        ca = entreprise('base_imposable_cstns_prestations_sans_abattement_droits', period)
