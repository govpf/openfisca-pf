...
class csis_net_reduction_taux_saisie(Variable):
    ...
    entity = Personne
    definition_period = DAY
    label = "Taux Net Reduction CSIS Saisie"


class csis_net_reduction_taux(Variable):
    value_type = float
    entity = Personne
    definition_period = DAY
    label = "Taux Net Reduction CSIS"

    def formula(personne: Population, period: Period, parameters: ParameterNode) -> ArrayLike:
        activite_principale = personne('is_activite_principale', period)
        possede_reduction = personne('csis_net_possede_reduction', period)
        reduction_taux_est_a_saisir = personne('csis_net_reduction_taux_est_a_saisir', period)
        reduction_saisie = personne('csis_net_reduction_taux_saisie', period)
        ...
        activites_croisieres = parameters(period).dicp.impot_societe.taux.csis_reduction.ACTIVITES_CROISIERE
        concessions_minieres = parameters(period).dicp.impot_societe.taux.csis_reduction.CONCESSIONS_MINIERES
        membre_groupe_fiscal = parameters(period).dicp.impot_societe.taux.csis_reduction.MEMBRE_GROUPE_FISCAL
        gie = parameters(period).dicp.impot_societe.taux.csis_reduction.GIE
        scpr = parameters(period).dicp.impot_societe.taux.csis_reduction.SCPR
        scm = parameters(period).dicp.impot_societe.taux.csis_reduction.SCM
        obnl = parameters(period).dicp.impot_societe.taux.csis_reduction.OBNL
